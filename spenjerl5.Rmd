...
Hint: To check stability,  partition the data and see how well clusters formed based on one part apply to the other part. To do this:
...
#Normalising data
dataA_norm <- scale(dataA_num)
dataB_norm <- scale(dataB_num)
...
#Taking mean
dataA_norm <- data.frame(dataA_norm)
new2 <- cbind(dataA_norm, clusters = points_hc)
center <-apply (dataA_norm, 2, function (x) tapply (x, new2$clusters, mean))
...
dataB_norm <- data.frame(dataB_norm)
cluster <- apply(dataB_norm, 1, closest.cluster)
dataBnew <- cbind(dataB_norm,cluster)

#Comparing dataframes 
ctable = compare_df(data_cluster, dataBnew, c("cluster"))
