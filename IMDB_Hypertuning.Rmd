---
title: "IMDB dataset-Hypertuning"
output:
...
#Divide training set into train and validation set for future validating purposes.
```{r}
val_indices <- 1:10000
...
units4_model%>% fit(train, train_label, epochs = 4, batch_size = 512)
...
units64_model%>% fit(train, train_label, epochs = 2, batch_size = 512)
...
units128_model%>% fit(train, train_label, epochs = 2, batch_size = 512)
...
```
# smaller hidden units of 4 model starts overfitting much later than initial model and when hidden units are increased to 64 or 128 the models overfits early like with 128 units loss is increased in second epoch itself.
...
plot(history_mse)
mse_model%>% fit(train, train_label, epochs = 2, batch_size = 512)
...
#In above graphs we can clearly notice that loss is less and accuracy is also better for mse as loss function model.
...
plot(history_tanh)
tanh_model%>% fit(train, train_label, epochs = 2, batch_size = 512)
...
#References:"Deep Learning with R"-Chollet Allaire.
#            https://towardsdatascience.com/intuitions-on-l1-and-l2-regularisation-235f2db4c261
#            Lecture notes-MIS64061,MIS64037.

#Differnt number ofHidden Layers 
# 3 hidden layers
```{r}
